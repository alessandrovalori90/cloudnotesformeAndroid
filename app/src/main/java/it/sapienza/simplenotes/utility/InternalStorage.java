...
public class InternalStorage {
    ...
    public static void writeNotesInternalStorage(GlobalClass global, Context context, String id){
        if(global.getList() == null) return;
        Gson gson = new Gson();
        String fileContents = gson.toJson(global.getList() );
        new File(context.getFilesDir(), context.getResources().getString(R.string.storage_notes_file_name)+id);
        FileOutputStream outputStream;
        try {
            outputStream = context.openFileOutput(context.getResources().getString(R.string.storage_notes_file_name)+id, Context.MODE_PRIVATE);
    ...
    public static Settings readSettingsInternalStorage(Context context, String id){
        FileInputStream fis;
        String line;
        StringBuilder sb = new StringBuilder();
        Settings settings;
        try {
            fis = context.openFileInput(context.getResources().getString(R.string.storage_settings_file_name)+id);
            InputStreamReader isr = new InputStreamReader(fis);
            BufferedReader bufferedReader = new BufferedReader(isr);
            while ((line = bufferedReader.readLine()) != null) {
                sb.append(line);
    ...
    public static void writeSettingsInternalStorage(GlobalClass global, Context context, String id){
        Gson gson = new Gson();
        String fileContents = gson.toJson(global.getSettings() );
        new File(context.getFilesDir(), context.getResources().getString(R.string.storage_settings_file_name)+id);
        FileOutputStream outputStream;
        try {
            outputStream = context.openFileOutput(context.getResources().getString(R.string.storage_settings_file_name)+id, Context.MODE_PRIVATE);
            outputStream.write(fileContents.getBytes());
            outputStream.close();
        } catch (Exception e) {
            e.printStackTrace();
